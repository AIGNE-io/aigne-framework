...
**问题**: 是否应该在 `--json` 之外增加 `--yaml` 输出格式？

**结论**: ❌ 不支持

**理由**:

1. **同构冗余** — YAML 和 JSON 表达能力相同，增加选择但不增加价值

2. **工具链成熟度** — jq 几乎所有系统都有，yq 需要额外安装

3. **LLM 友好性** — JSON 对 LLM 更友好（解析稳定、无缩进歧义、训练数据更多）

4. **四层架构无位置** — YAML 不比 JSON 更 machine-safe、更 schema-stable、更 token-cheap、更视觉友好，它是"第五层但没有独特价值"

5. **维护成本** — 需要确保 JSON/YAML schema 同步，测试用例翻倍
...
**例外情况**: 如果未来有强需求（大量用户反馈），可考虑作为输出格式转换（`--json --output-format=yaml`），而不是独立视图。
