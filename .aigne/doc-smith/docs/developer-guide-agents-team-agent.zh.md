本文档详细介绍了 `TeamAgent` 类，这是一个用于协调多个 Agent 协同工作的强大组件。文中涵盖了其配置、处理模式以及反思和迭代等特殊功能。

### 概述

`TeamAgent` 负责协调一组 Agent（称为“技能”）以完成复杂任务。它根据指定的处理模式管理这些 Agent 的执行，允许它们按顺序（一个接一个）或并行（一次性全部）工作。这对于构建复杂的工作流特别有用，在此类工作流中，一个 Agent 的输出可作为另一个 Agent 的输入，或者当需要同时执行多个任务以整合其结果时。
...
| `reflection` | `ReflectionMode` | 用于为团队输出启用迭代式审查和优化过程的配置。 | `undefined` |
...
| `includeAllStepsOutput` | `boolean` | 在顺序模式下，如果为 `true`，则所有中间步骤的输出都将包含在最终结果中。 | `false` |
...
#### `sequential`
在此模式下，Agent 将被逐个处理。每个 Agent 的输出将作为额外输入传递给序列中的下一个 Agent。这是默认模式，非常适合创建线性工作流。

#### `parallel`
在此模式下，所有 Agent 将同时处理。每个 Agent 接收相同的初始输入，其输出将被合并。此模式适用于可以独立执行然后合并结果的任务。

### 关键特性

#### 反思模式
反思模式提供了一种对团队输出进行迭代式自我修正和优化的机制。启用后，指定的 `reviewer` Agent 会评估团队的结果。如果输出未获批准，该过程将重复进行，并将前一次的输出和审查者的反馈作为下一次迭代的上下文。此循环将持续进行，直到输出获得批准或达到 `maxIterations` 限制。
...
| `isApproved` | `((output: Message) => PromiseOrValue<boolean \| unknown>) \| string` | 一个函数或审查者输出中的字段名，用于确定结果是否获得批准。真值表示批准。 |
| `maxIterations` | `number` | 停止前审查迭代的最大次数。默认为 `3`。 |
| `returnLastOnMaxIterations` | `boolean` | 如果为 `true`，当达到 `maxIterations` 时，Agent 将返回最后生成的输出，即使该输出未获批准。如果为 `false`，则会抛出错误。默认为 `false`。 |
...
`iterateOn` 功能使 `TeamAgent` 能够对输入消息中的数组执行批处理。当您使用 `iterateOn` 指定一个输入字段键时，Agent 会遍历该数组的每个元素，并通过团队的工作流单独处理每个元素。
...
根据配置的 `mode`，通过团队的 Agent 路由输入消息并进行处理。
