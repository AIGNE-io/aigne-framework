本文档提供了 `@aigne/lmstudio` 包的全面使用指南，这是一个 AIGNE 模型适配器，用于通过 LM Studio 与本地托管的 AI 模型进行集成。

## 概述

`@aigne/lmstudio` 模型适配器可与 LM Studio 的 OpenAI 兼容 API 无缝集成，让您可以通过 AIGNE 框架运行本地大语言模型（LLM）。LM Studio 提供了一个用户友好的界面，用于下载、管理和运行本地 AI 模型，并内置一个模仿 OpenAI API 的服务器。
...
2.  **下载模型**：使用 LM Studio 界面搜索并下载一个本地模型。热门选择包括 Llama 3.2、Mistral 和 Phi-3。
3.  **启动本地服务器**：在 LM Studio 中，导航至“Local Server”选项卡，选择您下载的模型，然后点击“Start Server”。这将开放一个本地端点（通常是 `http://localhost:1234/v1`），适配器将连接到该端点。

## 安装

使用您偏好的包管理器将该包安装到您的项目中：
...
您可以通过 `LMStudioChatModel` 的构造函数或使用环境变量来配置它。
...
```bash
# 设置 LM Studio 服务器 URL（默认：http://localhost:1234/v1）
...
**注意：** LM Studio 通常在本地运行，无需身份验证。API 密钥默认设置为占位符值 `"not-required"`，以满足底层的 OpenAI 客户端要求。
...
为了获得实时响应，您可以从模型中流式传输输出。这对于聊天机器人等希望在生成响应时就显示内容的应用非常有用。
...
该适配器支持与 OpenAI 兼容的函数调用，允许模型请求调用外部工具。
...
LM Studio 支持多种开源模型。配置中使用的模型名称必须与 LM Studio 界面中显示的名称完全匹配。热门选择包括：

-   **Llama 3.2**（3B、8B、70B 变体）
-   **Llama 3.1**（8B、70B、405B 变体）
-   **Mistral**（7B、8x7B 变体）
-   **CodeLlama**（7B、13B、34B 变体）
-   **Qwen**（多种大小）
-   **Phi-3**（mini、small、medium 变体）
...
1.  **连接被拒绝**：当 LM Studio 本地服务器未运行时，会发生此错误（`ECONNREFUSED`）。请确保您已在 LM Studio 应用程序的“Local Server”选项卡中启动了服务器。
2.  **模型未找到**：如果您收到“model not found”错误，请验证您配置中的 `model` 名称与 LM Studio 中加载的模型文件名完全匹配。
3.  **内存不足**：大型模型会消耗大量系统资源。如果您遇到崩溃或内存问题，请尝试使用较小的模型（例如，3B 或 8B 参数的变体）或减少上下文长度（`maxTokens`）。
4.  **响应缓慢**：响应速度取决于您的硬件（CPU/GPU）和模型大小。为了更快的推理速度，如果您的硬件支持，请考虑使用 GPU 加速，并在 LM Studio 中正确配置。