 ...
 * be omitted.
 */
Writer.prototype.renderTokens = async function renderTokens(
  tokens,
  context,
  partials,
  originalTemplate,
  ...
  for (let i = 0, numTokens = tokens.length; i < numTokens; ++i) {
    ...
    if (symbol === "#")
      value = await this.renderSection(
        token,
        context,
        partials,
        originalTemplate,
      ...
      value = await this.renderInverted(
        token,
        context,
        partials,
        originalTemplate,
...
Writer.prototype.renderSection = async function renderSection(
  token,
  context,
  partials,
  originalTemplate,
...
Writer.prototype.renderInverted = async function renderInverted(
  token,
  context,
  partials,
  originalTemplate,
...
Writer.prototype.unescapedValue = async function unescapedValue(
  token,
  context,
) {
  const value = await context.lookup(token[1]);
...
Writer.prototype.escapedValue = async function escapedValue(
  token,
  context,
  config,
) {
