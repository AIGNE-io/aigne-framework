...
`@aigne/aigne-hub` SDK 提供了一个统一的接口，用于访问各种 AI 模型以进行聊天和图像生成。它充当 [AIGNE Hub](https://github.com/AIGNE-io/aigne-framework) 的客户端，AIGNE Hub 是一个强大的代理层，连接到多个大型语言模型（LLM）提供商。

本指南将引导您完成该 SDK 的安装、基本设置以及在聊天补全和图像生成方面​​的使用。

## 简介

`@aigne/aigne-hub` 通过 AIGNE Hub 服务路由请求，简化了与各种 AI 提供商的交互。该网关聚合了 OpenAI、Anthropic、AWS Bedrock、Google 等提供商，让您只需更改模型标识符即可在它们之间无缝切换。这种方法抽象了处理不同 API 和身份验证方法的复杂性，让您可以专注于构建您的应用程序。

### 工作原理

SDK 将请求从您的应用程序发送到中心化的 AIGNE Hub 实例。然后，Hub 根据指定的模型名称将这些请求转发给相应的下游 AI 提供商。这种架构为所有 AI 交互提供了一个单一的访问和控制点。
...
-   🧠 **支持多提供商**：使用简单的 `provider/model` 命名约定，访问来自 OpenAI、Anthropic、Google 等的模型。
-   🔐 **安全身份验证**：使用单一的 `accessKey` 安全地管理 API 访问。
-   💬 **聊天补全**：使用 `{ role, content }` 消息格式的聊天模型标准化接口。
-   🎨 **图像生成**：使用来自 OpenAI (DALL-E)、Google (Imagen) 和 Ideogram 的模型生成图像。
-   🌊 **支持流式传输**：通过启用流式传输，获取聊天模型的实时、令牌级别的响应。
-   🧱 **兼容框架**：与更广泛的 AIGNE 框架无缝集成。
...
对于交互式、实时应用程序，您可以流式传输模型的响应。在 `invoke` 调用中将 `streaming` 选项设置为 `true`，并遍历生成的流以处理到达的数据块。
...
| `accessKey` | `string` | 用于向 AIGNE Hub 进行身份验证的秘密 API 密钥。 |
...
使用您的 Hub 凭据和所需的图像模型实例化 `AIGNEHubImageModel`。然后，使用提示词和其他模型特定参数调用 `invoke`。
...
-   **参考**：[OpenAI Images API Documentation](https://platform.openai.com/docs/guides/images)
...
-   **参考**：[Google GenAI Models Documentation](https://googleapis.github.io/js-genai/release_docs/classes/models.Models.html)
...
-   **参考**：[Ideogram API Documentation](https://developer.ideogram.ai/api-reference/api-reference/generate-v3)
