...
export class AFS implements AFSModule {
  ...
  async write(
    ...
    if (!module) throw new Error(`No module found for path: ${path}`);

    const entry = await module.module.write(module.subpath, content, options);
