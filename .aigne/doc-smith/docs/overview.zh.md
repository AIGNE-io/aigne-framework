本文档详细介绍了 `AIGNE` 类，它是 AIGNE 框架中的核心编排器。您将学习如何创建、配置和使用 `AIGNE` 实例来管理 Agent、处理消息传递以及构建复杂的 AI 驱动应用程序。
...
`AIGNE` 类的主要职责包括：
-   **Agent 管理**：加载、添加和管理构成应用程序的 Agent。
-   **执行上下文**：为每个工作流创建隔离的上下文，以管理状态并强制执行限制。
-   **调用**：提供灵活的 `invoke` 方法与 Agent 交互，支持标准响应和流式响应。
-   **消息传递**：为 Agent 之间的解耦通信提供发布-订阅系统。
-   **资源管理**：确保 Agent 及相关资源的正常关闭。

### 架构概览

`AIGNE` 类位于框架的核心，协调各种组件以执行复杂任务。下图说明了它在架构中的核心作用。
...
您可以通过两种主要方式创建 `AIGNE` 实例：直接使用其构造函数，或从文件系统加载配置。
...
| `limits` | `ContextLimits` | 执行上下文的使用限制，例如超时或最大 token 数。 |
...
对于更复杂的应用程序，您可以在 YAML 文件中定义您的 AIGNE 设置，并使用静态 `AIGNE.load()` 方法加载它。这种方法将配置与代码分离，使您的应用程序更加模块化。
...
`invoke` 方法是与 Agent 交互的主要方式。它支持多种模式，包括创建持久的用户会话、发送单条消息和流式响应。

**1. 创建用户 Agent**

在没有消息的情况下调用 Agent 会创建一个 `UserAgent`，该 `UserAgent` 维护一个一致的交互上下文。
...
要以块流的形式接收响应，请将 `streaming` 选项设置为 `true`。这对于像聊天机器人这样的实时应用程序非常理想。
...
您可以在 `AIGNE` 实例创建后动态地向其添加 Agent。该 Agent 将被附加到实例的生命周期和通信系统中。
...
该框架包含一个消息队列，用于 Agent 之间的解耦通信。Agent 可以向主题发布消息，其他 Agent 可以订阅这些主题以接收消息。
...
为确保干净地退出，`shutdown` 方法会正常终止所有 Agent 和技能，并清理它们持有的任何资源。
...
这也可以使用现代 JavaScript/TypeScript 中的 `Symbol.asyncDispose` 功能自动管理。
