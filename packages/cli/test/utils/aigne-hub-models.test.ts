...
import type { CheckModelResult, HubModel } from "@aigne/cli/utils/aigne-hub-models.js";
...
// Functions will be populated after dynamic import
let checkModelAvailability: (options: {
  baseUrl: string;
  apiKey: string;
  model: string;
}) => Promise<CheckModelResult>;

let fetchHubModels: (options: {
  baseUrl: string;
  apiKey: string;
  type?: string;
  search?: string;
...
describe("aigne-hub-models", () => {
  beforeAll(async () => {
    ...
    // Dynamic import to ensure our mock is in place
    const module = await import("@aigne/cli/utils/aigne-hub-models.js");
    checkModelAvailability = module.checkModelAvailability;
    fetchHubModels = module.fetchHubModels;
  });
  ...
  test("fetchHubModels should not include model param when no search provided", async () => {
    ...
    const calls = mockFetch.mock.calls as unknown as Array<[string, ...unknown[]]>;
    const callUrl = calls[calls.length - 1]?.[0];
