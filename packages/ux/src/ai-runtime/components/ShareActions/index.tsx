...
import {
  type RuntimeOutputShare,
  RuntimeOutputVariable,
  type RuntimeOutputVariablesSchema,
} from "@aigne/agent-v1";
...
function ShareTwitter({
  ...
  const query = useMemo(() => {
    const content =
      message.outputs?.objects?.find((i) => i?.[RuntimeOutputVariable.text])?.[
    ...
    <StyledActionButton
      ...
      onClick={async () => {
        if (content) {
          ...
          if (shareAttach.shareAttachUrl && link) {
            texts.push(link);
          }
