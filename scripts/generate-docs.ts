...
await Promise.all(
  entries.map(async ({ sources, examples, output }) => {
    ...
    const zhOutputPath = outputPath.replace(/\.md$/, ".zh.md");

    await $({
      stdio: "inherit",
    })`aigne-wiki --input-language zh_CN --input-sources ${join(root, sources)} --input-examples ${join(root, examples)} --output-key markdown --output ${outputPath} --force`;

    await $({
      stdio: "inherit",
    })`aigne-wiki --entry-agent translate --input-language zh_CN --input-content @${outputPath} --output-key translation --output ${zhOutputPath} --force`;
  }),
);
