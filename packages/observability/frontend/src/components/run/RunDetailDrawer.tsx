...
export default function RunDetailDrawer({ open, onClose, run }: RunDetailDrawerProps) {
  ...
  const getRunStats = (run: RunData | null) => {
    ...
    function traverse(node: RunData | null) {
      if (!node) return;
      count += 1;
      if (node.output?.usage) {
        inputTokens += node.output.usage.inputTokens || 0;
        outputTokens += node.output.usage.outputTokens || 0;
      }
      ...
      <Box sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
        ...
        >
          ...
          <Box sx={{ textAlign: "center" }}>
            ...
            <Typography>{parseDuration(run.startedAt, run.endedAt)}</Typography>
          </Box>
        </Box>
