...
const [scriptName, ...argv] = hideBin(process.argv);

const appName = process.env.AIGNE_APP_NAME;
const appPackageName = process.env.AIGNE_APP_PACKAGE_NAME;
const appUseBetaVersion = process.env.AIGNE_APP_USE_BETA_APPS === "1" ? true : undefined;
...
try {
  ...
  if (!isUpgradeCommand(argv)) {
    ...
    y.version(`${appName} v${version}`).alias("version", "v");
  }
