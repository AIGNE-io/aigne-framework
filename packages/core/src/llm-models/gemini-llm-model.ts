  ...
  async *process(input: LLMModelInputs) {
    ...
    for await (const chunk of res.stream) {
      ...
      if (choice?.content.parts) {
        ...
        throw new Error(
          ['PROMPT_BLOCKED', blockReason, blockReasonMessage].filter(Boolean).join(' ')
        )
      }
    }
  }
...
async function contentsFromInputMessages(messages: LLMModelInputMessage[]): Promise<Content[]> {
...
function generationConfigFromInput(input: LLMModelInputs): GenerationConfig {
  const jsonSchema =
    input.responseFormat?.type === 'json_schema' ? input.responseFormat.jsonSchema : undefined
...
function toolsFromInputTools(tools?: LLMModelInputs['tools']): Tool[] | undefined {
  return tools?.length
    ? [
        {
          functionDeclarations: tools.map(i => ({
            ...
            parameters:
              !i.function.parameters || Object.keys(i.function.parameters).length === 0
                ? undefined
...
function openAISchemaToGeminiSchema(schema: {[key: string]: any}): ResponseSchema {
