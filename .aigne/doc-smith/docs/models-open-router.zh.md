本文为集成 `@aigne/open-router` 包的开发者提供了一份全面的指南。您将学习如何安装、配置和使用此包，以通过统一的接口利用各种 AI 模型。
...
`@aigne/open-router` 提供了 AIGNE 框架与 OpenRouter 统一 API 之间的无缝集成。这使得开发者可以通过单一、一致的接口访问来自 OpenAI、Anthropic 和 Google 等提供商的大量 AI 模型，从而简化了模型选择并实现了强大的回退配置。
...
*   **模型回退**：当主模型失败时，自动切换到备用模型。
*   **流式支持**：通过流式响应，实现实时、响应迅速的应用程序。
*   **AIGNE 框架兼容性**：与 `@aigne/core` 模型接口完美集成。
*   **丰富的配置**：通过广泛的选项微调模型行为。
...
此包的主要导出是 `OpenRouterChatModel`。它扩展了 `@aigne/openai` 包的 `OpenAIChatModel`，因此接受相同的选项。

要配置模型，您需要提供您的 OpenRouter API 密钥。您可以通过直接将其传递给构造函数或设置 `OPEN_ROUTER_API_KEY` 环境变量来完成此操作。
...
对于需要实时交互的应用程序，您可以启用流式传输以在生成响应块时接收它们。在 `invoke` 方法中设置 `streaming: true` 选项。
...
console.log(fullText); // 输出: "我由 OpenRouter 提供支持，使用的是 Anthropic 的 Claude 3 Opus 模型。"
console.log(json); // { model: "anthropic/claude-3-opus", usage: { inputTokens: 5, outputTokens: 14 } }
```

## 使用带回退功能的多个模型

`@aigne/open-router` 的一个关键功能是能够配置回退模型。如果主模型因任何原因（例如 API 错误、速率限制）失败，系统将自动尝试指定列表中的下一个模型。

您可以使用 `fallbackModels` 选项定义回退顺序。
