...
execSync(
  "bumpp --no-tag --no-commit --no-push package.json packages/*/package.json",
  {
    stdio: "inherit",
  },
);
...
console.log(
  chalk.greenBright(`[info]: start to modify packages version to ${version}`),
);
const dirPath = path.join(__dirname, "../packages");
let changeLogDirs = fs.readdirSync(dirPath);
changeLogDirs = [
  join(__dirname, ".."),
  ...changeLogDirs.map((item) => `${dirPath}/${item}`),
];
